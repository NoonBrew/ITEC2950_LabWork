...
class Video(models.Model):
    ...
    def save(self, *args, **kwargs):
        # Extract the video id from a youtube url and raise exceptions if it is not a valid url
        if not self.url.startswith('https://www.youtube.com/watch'):
            raise ValidationError(f'Not a YouTube URL {self.url}')

        url_components = parse.urlparse(self.url)
        query_string = url_components.query # 'v=asbakd2312'
        if not query_string:
            raise ValidationError(f'Invalid YouTube URL {self.url}')
        parameters = parse.parse_qs(query_string, strict_parsing=True) # Dictionary
        v_parameters_list = parameters.get('v') # Returns None if no key found, e.g. abc=1234
